...
class UniversityManagementSystem {
    ...
    ensureRegistrationMaps(studentId, courseId) {
    ...
    addCourse(course) {
        ...
        if (course.maxStudents <= 0)
            throw new Error("maxStudents must be > 0");
        const withId = { id: this.courseIdSeq++, ...course };
    ...
    enrollStudent(student) {
        ...
        if (student.year < 1 || student.year > 6)
            throw new Error("year must be 1..6");
        if (!(student.enrollmentDate instanceof Date))
            throw new Error("enrollmentDate must be Date");
        if (!student.groupNumber?.trim())
            throw new Error("groupNumber is required");
        const withId = { id: this.studentIdSeq++, ...student };
    ...
    registerForCourse(studentId, courseId) {
        ...
        if (courseSet.has(studentId))
            return; // вже зареєстрований — ок, без дубля
    ...
    setGrade(studentId, courseId, grade) {
        ...
        const idx = this.grades.findIndex((g) => g.studentId === studentId && g.courseId === courseId);
        if (idx >= 0)
    ...
    getAvailableCourses(faculty, semester) {
        return this.courses.filter((c) => {
            if (c.faculty !== faculty || c.semester !== semester)
                return false;
...
try {
    ...
    console.log("Graduate error:", e.message);
}
// Випуск ст1
